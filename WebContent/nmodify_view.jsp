...
<script type="text/javascript" src="./naver-editor/js/service/HuskyEZCreator.js" charset="utf-8"></script>
 	<script>
        function nform_check() {
        	oEditors.getById["bContent"].exec("UPDATE_CONTENTS_FIELD", []);

    	    document.nmodify_form.submit();
        }
    </script>
...
<div id="content">
<div id="main_content">
<div style="text-align:center"><h2>공지 게시글 정보 수정</h2></div>
<div style="text-align:center"><h5>공지 게시글 정보를 수정합니다.</h5></div>
<table class="table">
	<form name="nmodify_form" action="nmodify.do" method="post">
	<input type="hidden" name="bId" value="${notice_view.bId }">	
	<tr>
		<td scope="row" style="text-align:center">번호</td>
		<td> ${notice_view.bId }</td>
	</tr>
	<tr>
		<td scope="row" style="text-align:center">히트</td>
		<td>${notice_view.bHit }</td>
	</tr>
	<tr>
		<th scope="row" style="text-align:center" >이름</th>		
		<td><input type="hidden" name="bName" value="${notice_view.bName }">${notice_view.bName }</td>
	</tr>
	<tr >
		<th scope="row" style="text-align:center" >제목</th>
		<td> <input type="text" name="bTitle" value="${notice_view.bTitle }"></td>
	</tr>
	<tr >
		<th scope="row" style="text-align:center" >내용</th>
		<td> <textarea name="bContent" id="bContent" rows="10"  cols="100">${notice_view.bContent }</textarea> </td>
	...
	<tr align="right">
		...
		<a href="JavaScript:nform_check();">수정완료</a> &nbsp;&nbsp;
		<a href="notice_view.do?bId=${notice_view.bId }">취소</a>&nbsp;&nbsp;
