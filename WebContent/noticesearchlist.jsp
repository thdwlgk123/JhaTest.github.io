...
<div id="content">
<div id="main_content">
<div style="text-align:center"><h2>공지글 검색</h2></div>
<div style="text-align:center"><h5>특정 공지글 검색 결과를 보여줍니다.</h5></div>
<table class="table" >
	<thead>
	...
	<tr>
		...
		<c:otherwise>
			<a  href="noticesearch.do?page=1&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&lt;&lt;]</a>
		</c:otherwise>
		</c:choose>
		...
		<c:otherwise>
			<a  href="noticesearch.do?page=${notice_page.curPage-1}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&lt;]</a>
		</c:otherwise>
		</c:choose>
		<!-- 개별페이지 -->
		<c:forEach var="fEach" begin="${notice_page.startPage }" end="${notice_page.endPage }" step="1">
			...
			<c:otherwise>
				<a  href="noticesearch.do?page=${fEach}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[${fEach }]</a>&nbsp;
			</c:otherwise>
			</c:choose>
		</c:forEach>
		<!-- 다음 -->
		<c:choose>
		<c:when test="${(notice_page.curPage+1)>notice_page.totalPage }">
			[&gt;]
		</c:when>
		<c:otherwise>
			<a href="noticesearch.do?page=${notice_page.curPage+1 }&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&gt;]</a>
		...
		<c:otherwise>
			<a href="noticesearch.do?page=${notice_page.totalPage}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&gt;&gt;]</a>
		</c:otherwise>
		</c:choose>	
		</td>
	</tr>				
...
<tr>
	<td align="center">
<% 
if(request.getAttribute("searchtype").equals("bTitle")){
%>
	<select name="searchtype" id="searchtype">
		<option name="title" value="title" selected="selected">제목</option>
...
<%
 ...
 %>		
	<select name="searchtype" id="searchtype">
		...
		<option name="content" value="content" selected="selected">내용</option>
	</select>
<% 	} %> 
