...
<body>
 ...
 <div id="content">
<div id="main_content">
<div style="text-align:center"><h2>게시글 찾기</h2></div>
<table class="table">
	<thead>
	...
	<tr>
		...
		<c:otherwise>
			<a  href="search.do?page=1&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&lt;&lt;]</a>
		</c:otherwise>
		</c:choose>
		...
		<c:otherwise>
			<a  href="search.do?page=${page.curPage-1}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&lt;]</a>
		</c:otherwise>
		</c:choose>
		<!-- 개별페이지 -->
		<c:forEach var="fEach" begin="${page.startPage }" end="${page.endPage }" step="1">
			...
			<c:otherwise>
				<a  href="search.do?page=${fEach}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[${fEach }]</a>&nbsp;
			</c:otherwise>
			</c:choose>
		</c:forEach>
		<!-- 다음 -->
		...
		<c:otherwise>
			<a href="search.do?page=${page.totalPage}&searchtext=<%=request.getAttribute("searchtext") %>&searchtype=<%=request.getAttribute("searchtype")%>">[&gt;&gt;]</a>
		</c:otherwise>
		</c:choose>	
		</td>
	</tr>	
...
<%
 	} else if(request.getAttribute("searchtype").equals("bContent")){
%>
	<select name="searchtype" id="searchtype">
